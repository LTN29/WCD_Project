...
public class CategoryDAO {
	...
	public static List<Category> getAll() throws SQLException {
		...
		try (Connection conn = DBUtil.getInstance().getConnect();
			...
			while (rs.next()) {
				Category c = new Category(rs.getInt("_id"), rs.getString("_name"), rs.getBoolean("_active"));
				list.add(c);
			}
		}
		return list;
	...
	public static Category getById(int id) throws SQLException {
		...
		try (Connection conn = DBUtil.getInstance().getConnect(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
				return new Category(rs.getInt("_id"), rs.getString("_name"), rs.getBoolean("_active"));
			}
		}
		return null;
	}
