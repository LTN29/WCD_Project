...
public class StoryCommentDAO {
	...
	public static int countPendingComments(long userId, int storyId) throws SQLException {
		String sql = "SELECT COUNT(*) FROM tbl_story_comment WHERE _user_id = ? AND _story_id = ? AND _active = 0";
		try (Connection conn = DBUtil.getInstance().getConnect(); PreparedStatement stmt = conn.prepareStatement(sql)) {
