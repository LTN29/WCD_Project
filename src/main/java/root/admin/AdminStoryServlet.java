...
public class AdminStoryServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String sort = req.getParameter("sort"); // like | follow | view | null
        ...
        try {
            if ("edit".equals(action)) {
                ...
                switch (sort == null ? "" : sort) {
                    case "like" -> orderBy = "_like_number";
                    case "follow" -> orderBy = "_follow_number";
                    case "view" -> orderBy = "_view_number";
                    default -> orderBy = "_id"; // hoặc "_created_at"
                }

                List<Story> stories;

                if (keyword != null && !keyword.trim().isEmpty()) {
                    stories = StoryDAO.searchStories(keyword.trim(), orderBy);
                    req.setAttribute("keyword", keyword);
                    req.setAttribute("sort", sort);
                } else {
                    stories = StoryDAO.getAllWithNamesSorted(orderBy);
                }
